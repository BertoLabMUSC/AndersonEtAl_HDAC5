...
pdf("d1_dge_onebatch/PCA_D1_onebatch.pdf",width=6,height=6,useDingbats=FALSE)
...
save(D1_FullTab_onebatch,D1_DGE_onebatch, file = "d1_dge_onebatch/D1_Dge_Data_onebatch.RData")


openxlsx::write.xlsx(D1_FullTab_onebatch, 
                     file = "d1_dge_onebatch/D1_DGE_FullStats_onebatch.xlsx", 
                     colNames = TRUE,
                     rowNames = FALSE, 
                     borders = "columns",
                     sheetName="Stats",
...
openxlsx::write.xlsx(D1_DGE_onebatch, 
                     file = "d1_dge_onebatch/D1_DGE_onebatch.xlsx", 
                     colNames = TRUE,
                     rowNames = FALSE, 
                     borders = "columns",
                     sheetName="Stats",
...
write.table(p_regressed,"d1_dge_onebatch/expression_regressed_onebatch.txt",sep="\t",quote=F)

pdf("d1_dge_onebatch/PCA_D1_AdjustedForConfound_onebatch.pdf",width=6,height=6,useDingbats=FALSE)
...
pdf("d1_dge_onebatch/Boxplots_TopGenes_D1_onebatch.pdf",width=6,height=5,useDingbats=FALSE)
...
pdf("d1_dge_onebatch/Vulcano_Plot_D1_onebatch.pdf",width=6,height=6,useDingbats=FALSE)
ggscatter(df, 
            x = "logFC", 
            y = "LOG",
            color = "Threshold",
...
# heatmap
mat <- p_regressed[rownames(p_regressed)%in% D1_DGE$Gene,]
anno <- pd
Genotype        <- c("black", "red")
names(Genotype) <- c("WT", "HDAC5")
anno_colors <- list(Genotype = Genotype)
pdf("d1_dge_onebatch/Heatmap_D1_onebatch.pdf",width=4,height=6)
