...
# How to run
#load("NoD2ess_dge_output/NoD2ess_Dge_Data.RData")
mod=read.table("d2_dge/D2_Dge_HumanID.txt",sep="\t",header=T)
...
dir.create("d2_dge/GO_D2/")
dir.create("d2_dge/GO_D2/GO_BP/")
dir.create("d2_dge/GO_D2/GO_MF/")
dir.create("d2_dge/GO_D2/GO_CC/")

net=levels(mod$Direction)
for (i in 1:length(dfBP)) write.table(dfBP[[i]], file = paste("d2_dge/GO_D2/GO_BP/BP_", net[[i]], ".txt",sep = ""),sep="\t",quote=F)
for (i in 1:length(dfMF)) write.table(dfMF[[i]], file = paste("d2_dge/GO_D2/GO_MF/MF_", net[[i]], ".txt",sep = ""),sep="\t",quote=F)
for (i in 1:length(dfCC)) write.table(dfCC[[i]], file = paste("d2_dge/GO_D2/GO_CC/CC_", net[[i]], ".txt",sep = ""),sep="\t",quote=F)
...
pdf("BP_D2_enrichment.pdf",width=8,height=3,useDingbats=FALSE)
...
pdf("MF_D2_enrichment.pdf",width=8,height=3,useDingbats=FALSE)
ggscatter(top_labelled, x = "log", y = "OddsRatio",
   color = "Class", palette = c("royalblue", "royalblue", "royalblue"),size = 2,
   label = "Term", repel = TRUE,font.label = c(8, "plain"))+
xlab("-log10(p-value)")+ 
...
pdf("CC_D2_enrichment.pdf",width=8,height=3,useDingbats=FALSE)
ggscatter(top_labelled, x = "log", y = "OddsRatio",
   color = "Class", palette = c("royalblue", "royalblue", "royalblue"),size = 2,
   label = "Term", repel = TRUE,font.label = c(8, "plain"))+
xlab("-log10(p-value)")+ 
