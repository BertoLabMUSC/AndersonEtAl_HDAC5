...
MGI = biomaRt::getLDS(attributes = c("hgnc_symbol"), 
						filters = "hgnc_symbol", 
						values = rownames(exprSC), 
						mart = human, 
						attributesL = c("mgi_symbol","gene_biotype"), 
						martL = mouse, 
						uniqueRows=T)

exprSC_filt <- full_join(exprSC %>%
                           mutate(Symbol = rownames(exprSC)),MGI, by = c("Symbol" = "HGNC.symbol")) %>%
...
save(scSet,D1D2Set, file = "deconvolution/Deconvolution_Jaffe_Input.RData")
...
save(D1D2_Deconv,file = "deconvolution/D1D2_Jaffe_Deconv.RData")
...
pdf("deconvolution/Deconvolution_Jaffe.pdf",width=8,height=5)
ggboxplot(df, "variable", "value",
color = "Cell")+
ggtitle("Cell Type Proportion: WM4")+
xlab("")+ 
ylab("Proportion")+
